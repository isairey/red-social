...
//dando problemas
	public static function sel($user_id,$friend_id){
		$sql = "select (id) from ".self::$tablename." where (sender_id=$user_id and receptor_id=$friend_id) or (receptor_id=$user_id and sender_id=$friend_id) ";
		$query = Executor::doit($sql);
		return Model::one($query[0],new FriendData());
	}
...
//quedo bien
public static function eliminar($user_id,$friend_id){
		$sql = "delete  from ".self::$tablename." where (sender_id=$user_id and receptor_id=$friend_id) or (receptor_id=$user_id and sender_id=$friend_id) ";
		$query = Executor::doit($sql);
		//return Model::one($query[0],new FriendData());
	}
